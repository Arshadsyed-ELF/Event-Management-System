...
const Hosts= () => {
  ...
  const toggleDetail = () => {
   ...
   useEffect(() => {
    axios.get(`http://localhost:7000/hosts`)
      .then((response) => {
...
const deleteData = (taskId) => {
    axios.delete(`http://localhost:7000/hostdelete/${taskId}`);
    window.location.assign('/hosts');
    alert('User is deleted');
  };
  const deleteitem = (taskId) => {
    axios.delete(`http://localhost:7000/usereventdelete/${taskId}`);
    window.location.assign('/hosts');
    alert('deleted');
  };
  ...
  const fetchUserBikeData = (userId) => {
    ...
    axios.get(`http://localhost:7000/getevents/${userId}`)
  ...
  return (
    ...
    <Anavbar/>
    <br />
    <h1 className='text-center'>Hosts</h1> <br />
                    ...
                    <div className="bg-white p-4 rounded-lg z-10 relative" style={{ maxHeight: "80vh", overflowY: "scroll" }}>
                      ...
                      <p className="text-sm text-gray-600">
                        <div className="container mx-auto mt-8" style={{width:"1350px"}}>
                          <h1 className='text-center text-blue-300'> Events</h1>
                            ...
                            return (
                              ...
                              >
                                <div style={{ display: 'flex', justifyContent: 'space-around' }}>
                                  <div>
                                    <img src={`http://localhost:7000/${item?.eventImage}`} style={{ height: "80px",width:"120px" }} /> <br/>
                                  </div>
                                  <div>
                                    <p>Event Name:</p>
                                    <p>{item.eventName}</p>
